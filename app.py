...
with st.sidebar:
    ...
    with col1:
        max_tmj = st.slider("最大关节盘应力(MPa)", 3.0, 8.0, 5.0, key='max_tmj')
        min_mp = st.slider("最小前伸量(%)", 40, 70, 50, key='min_mp')
    with col2:
        max_pdl = st.slider("最大牙周膜应力(kPa)", 3.0, 9.0, 6.0, key='max_pdl')
...
if st.session_state.calculate:
    ...
    # 约束条件
    constraints = [
        {'type': 'ineq', 'fun': lambda x: st.session_state.max_tmj - tmj_stress(x, x[1])},
    ...
    with col1:
        st.header("📊 优化结果")
        st.metric("最佳前伸量(MP)", f"{result.x:.1f}%", delta="关节盘安全阈值" if result.x < 65 else "注意：接近高风险区")
        st.metric("最佳开口量(VO)", f"{result.x:.1f}mm", delta="牙周膜安全阈值" if result.x < 6 else "注意：接近高风险区")
        st.divider()
        ...
        st.metric("关节盘应力", f"{tmj_value:.2f} MPa", 
                 delta="安全" if tmj_value < 4 else "注意：接近危险值", delta_color="normal")
        st.metric("牙周膜应力", f"{pdl_value:.2f} kPa",
                 delta="安全" if pdl_value < 5 else "注意：接近危险值", delta_color="normal")
    ...
    if tmj_value < 4 and pdl_value < 5:
