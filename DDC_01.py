...
# DDC 损失计算
class DualBranchCNNLSTMModelWithDDC(nn.Module):
    def __init__(self, cnn_input_size, lstm_input_size, lstm_hidden_size, lstm_num_layers, num_classes):
        super(DualBranchCNNLSTMModelWithDDC, self).__init__()
    ...
    def forward(self, x1, x2):
        ...
        return class_output, out  # 返回特征用于 DDC 损失计算

    def ddc_loss(self, src_features, tgt_features):
        # 计算源域特征和目标域特征的均值
        src_mean = torch.mean(src_features, dim=0)
        tgt_mean = torch.mean(tgt_features, dim=0)

        # 计算 DDC 损失（例如，使用均方误差）
        ddc_loss = torch.mean((src_mean - tgt_mean) ** 2)
        return ddc_loss
...
model = DualBranchCNNLSTMModelWithDDC(input_size, input_size, lstm_hidden_size, lstm_num_layers, num_classes).to(device)
...
train_data, _ = train_test_split(source_dataset, test_size=0.2, random_state=42)
...
for epoch in range(total_epochs):
    ...
    for i, ((inputs1, inputs2), labels) in enumerate(train_loader):
        ...
        # 使用源域特征计算 DDC 损失
        src_features = src_features.detach()  # 只需要计算 DDC 损失
        tgt_features = src_features.clone().detach()  # 目标域特征（这里只是一个示例，需要替换为实际的目标域特征）
        ddc_loss = model.ddc_loss(src_features, tgt_features)

        total_loss = class_loss + ddc_loss
        total_loss.backward()
        optimizer.step()
...
for epoch in range(total_epochs):
    ...
    for i, ((inputs1, inputs2), labels) in enumerate(target_loader):
        ...
        # 计算 DDC 损失
        src_features = torch.zeros_like(tgt_features)  # 源域特征的均值，用于计算 DDC 损失
        ddc_loss = model.ddc_loss(src_features, tgt_features)
