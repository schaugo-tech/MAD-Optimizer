...
if st.session_state.calculate:
    ...
    if result.success:
        ...
        with col1:
            ...
            ########################################################
            st.metric("深度学习模型预测关节盘应力", f"{tmj_value*0.98:.2f} kPa",
                     delta="安全" if pdl_value < 4.7 else "注意：接近危险值", delta_color="normal")

            st.metric("深度学习模型预测牙周膜应力", f"{pdl_value*1.0101:.2f} kPa",
                     delta="安全" if pdl_value < 4.7 else "注意：接近危险值", delta_color="normal")
        ...
        with col2:
            ...
            surf1 = ax1.plot_surface(MP * 0.02 * st.session_state.max_mp_val, VO, tmj_stress(MP, VO), cmap='viridis',
                                     alpha=0.8)
            ax1.scatter(st.session_state.max_mp_val * 0.01 * result.x[0], result.x[1],
                        tmj_stress(result.x[0], result.x[1]), c='red', s=300, marker='*')
            ax1.set_title('Predicted TMJ Disc')

                # ================= 牙周膜应力子图 =================
            ax2 = fig.add_subplot(224, projection='3d')
            surf2 = ax2.plot_surface(MP * 0.02 * st.session_state.max_mp_val, VO, pdl_stress(MP, VO), cmap='plasma',
                                     alpha=0.8)
            ax2.scatter(st.session_state.max_mp_val * 0.01 * result.x[0], result.x[1],
                        pdl_stress(result.x[0], result.x[1]), c='blue', s=300, marker='^')
            ax2.set_title('Predicted Mandibular Anterior PDL')
