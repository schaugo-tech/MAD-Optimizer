...
for epoch in range(total_epochs):
    ...
    for i, ((inputs1, inputs2), labels) in enumerate(train_loader):
        ...
        # 调整 inputs2 的形状以匹配 LSTM 的输入
        inputs2 = inputs2.permute(0, 2, 1)  # (batch_size, seq_len, input_size)
...
with torch.no_grad():
    for (inputs1, inputs2), labels in test_loader:
        ...
        inputs2 = inputs2.unsqueeze(-1)  # 增加时间步维度
        inputs2 = inputs2.permute(0, 2, 1)  # 调整形状 (batch_size, seq_len, input_size)
        outputs = model(inputs1, inputs2)
